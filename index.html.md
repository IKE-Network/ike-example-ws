...
canonical_url: https://github.com/IKE-Network/ike-platform/ike-example-ws/index.html
---

# ike-example-ws

A workspace aggregator that orchestrates the IKE consumer-side release cascade end-to-end. Cloning this repo and running `mvn ws:init` pulls down `ike-docs`, `ike-platform`, `doc-example`, and `example-project` at the versions declared in `workspace.yaml`. A subsequent `mvn clean install` then rebuilds the full cascade in dependency order from a single command. This is the canonical template for an **IKE workspace aggregator**.
...
`workspace.yaml` declares the four subprojects this workspace manages. They are pulled in dependency order:

| Subproject | Live site |
| --- | --- |
| `ike-docs` | [ike.network/ike-docs/](https://ike.network/ike-docs/)[3] — Doc plumbing: `ike-doc-maven-plugin`, `koncept-asciidoc-extension`, `minimal-fonts`, `docbook-xsl`, `ike-doc-resources`, `semantic-linebreak`. |
| `ike-platform` | [ike.network/ike-platform/](https://ike.network/ike-platform/)[4] — Consumer-facing parent: `ike-parent`, `ike-bom`, `ike-workspace-maven-plugin`. |
| `example-project` | [ike.network/example-project/](https://ike.network/example-project/)[1] — JAR + docs consumer demo. |
| `doc-example` | [ike.network/doc-example/](https://ike.network/doc-example/)[2] — Doc-only consumer demo. |

`ike-tooling` is intentionally **not** a workspace subproject — it is the bootstrap project (the source of `ike-maven-plugin`, which every other subproject uses at build time) and is released independently. See the comment block in `workspace.yaml` for the rationale.
...
For the full reference of all 50 `ws:*` goals, see the [ws:* Goal Reference](https://ike.network/ike-platform/ike-workspace-maven-plugin/ws-goals.html)[5] on the IKE Platform site. For first-time setup of a workspace, see [Workspace Getting Started](https://ike.network/ike-platform/workspace-getting-started.html)[6].
...
2. **All subprojects track the workspace’s git branch.** No per-subproject branch opt-out. See [ike-issues#233](https://github.com/IKE-Network/ike-issues/issues/233)[7] for the alignment model.
...
| Resource | URL |
| --- | --- |
| Source repository | [https://github.com/IKE-Network/ike-example-ws](https://github.com/IKE-Network/ike-example-ws)[8] |
...
| IKE Platform (workspace plugin lives here) | [https://ike.network/ike-platform/](https://ike.network/ike-platform/)[4] |
| ws:* Goal Reference | [https://ike.network/ike-platform/ike-workspace-maven-plugin/ws-goals.html](https://ike.network/ike-platform/ike-workspace-maven-plugin/ws-goals.html)[5] |
| Workspace Getting Started | [https://ike.network/ike-platform/workspace-getting-started.html](https://ike.network/ike-platform/workspace-getting-started.html)[6] |
