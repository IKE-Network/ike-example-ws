...
The corollary: when the foundation repos release (e.g., ike-tooling 150 → ike-docs 12 → ike-platform 34), this workspace picks up the new versions by bumping `ike-parent’s version (via `ws:set-parent-publish -Dparent.version=34`) and any direct `151` / `13` references — but the foundation repos themselves are never under `ws:*` control.
