...
The `its/` reactor is the **executable specification** for cross-cascade behavior in the IKE Network release stack. It runs the four-repo (or five-repo, including this workspace) build chain the way an external consumer would — pulling artifacts from Nexus, scaffolding a fresh project layout, and asserting that the documented pipeline actually works end-to-end.

## [#why-an-it-suite-at-this-layer](#why-an-it-suite-at-this-layer)Why an IT suite at this layer?

The four foundation reactors (ike-tooling, ike-docs, ike-platform, plus this workspace’s two consumers doc-example and example-project) each have their own unit and integration tests inside their own modules. Those catch regressions inside a single reactor.
...
Each of those is testable inside a fresh `maven-invoker-plugin` sandbox, and each represents a real regression the IKE Network has hit at least once during the v144→v149 cascade work.

## [#planned-it-inventory](#planned-it-inventory)Planned IT inventory

The cases below are listed in the suite’s `README.adoc` as the intended coverage. Implementations are pending — `src/it/` currently holds only `settings.xml`. Each case is small (a handful of files) and runs in seconds when the artifacts under test are present in the local repository.
...
- The version-nested staging recursion (#337) — would have been caught by a `release-cascade` IT exercising `1` substitution in `site.deploy.url`.
...
- [ike-example-ws overview](index.html)[1] — workspace landing page.
- [workspace.yaml — Annotated Tour](workspace-yaml.html)[2] — the manifest format that the IT suite assumes the workspace exercises.
- [maven-invoker-plugin](https://maven.apache.org/plugins/maven-invoker-plugin/)[3] — upstream documentation for the harness.
